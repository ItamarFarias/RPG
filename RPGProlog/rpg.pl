...
playerMaker(3, Nome):- assert(player(Nome, 250, 250, 80, 80, 45, 20, 30, 15, 100, 0, 1, 100, true)). % Ca√ßador
...
class(3, Nome, X):- string_concat(Nome, " -- (Classe : Ca√ßador) ", X).
class(4, Nome, X):- string_concat(Nome, " -- (Classe : Mago) ", X).

%Implementa√ß√£o do comando/regra if/then/else. O uso do cut "!" significa que o programa nao ira mais fazer buscas por outra regra if / n vai atras de queries mais.
if(Condition,Then,Else) :- Condition, !, Then.
if(_,_,Else) :- Else.

%Provavelmente deve ser modificado para usar tamb√©m o retract para excluir o fato que representa o player existente!
...
%Reduz a quantidade de mana do player, que √© definido como um fato. Logo, temos de reescrever o fato. -Regra usada na cura.
...
% regras auxiliares para criaÁ„o de inimigos.
